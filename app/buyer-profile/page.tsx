...
export default function BuyerProfilePage() {
  ...
  const handleSubmit = async () => {
    ...
    alert('Thank you! Your buyer profile has been submitted. We\'ll contact you within 24 hours.')
  }
            ...
            {['Ostuni', 'Monopoli', 'Polignano a Mare', 'Locorotondo', 'Alberobello', 'Cisternino', 'Martina Franca', 'Fasano', 'Other'].map(location => (
              <label key={location} className="flex items-center">
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      updateFormData('location', [...formData.location, location])
                    } else {
                      updateFormData('location', formData.location.filter(l => l !== location))
                    }
                  }}
            ...
            {['Property Search', 'Legal Process', 'Financing', 'Renovation', 'Property Management', 'Tax Planning', 'Residency/Visa', 'Local Connections'].map(area => (
              <label key={area} className="flex items-center">
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      updateFormData('needsAssistance', [...formData.needsAssistance, area])
                    } else {
                      updateFormData('needsAssistance', formData.needsAssistance.filter(a => a !== area))
                    }
                  }}
                  className="mr-2 rounded border-gray-300 text-green-600 focus:ring-green-500"
                />
            ...
            {['Building Insurance', 'Contents Insurance', 'Liability Insurance', 'Rental Income Protection', 'Title Insurance', 'Natural Disaster Coverage'].map(insurance => (
              <label key={insurance} className="flex items-center">
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      updateFormData('insuranceNeeds', [...formData.insuranceNeeds, insurance])
                    } else {
                      updateFormData('insuranceNeeds', formData.insuranceNeeds.filter(i => i !== insurance))
                    }
                  }}
                  className="mr-2 rounded border-gray-300 text-green-600 focus:ring-green-500"
                />
  ...
  const renderGrantEligibility = () => (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900">Grant Eligibility</h2>
            ...
            {['Solar Panels', 'Rainwater Harvesting', 'Geothermal Heating', 'Energy Efficient Windows', 'Organic Farming', 'EV Charging Station', 'Waste Recycling System'].map(feature => (
              <label key={feature} className="flex items-center">
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      updateFormData('sustainabilityFeatures', [...formData.sustainabilityFeatures, feature])
                    } else {
                      updateFormData('sustainabilityFeatures', formData.sustainabilityFeatures.filter(f => f !== feature))
                    }
                  }}
                  className="mr-2 rounded border-gray-300 text-green-600 focus:ring-green-500"
                />
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-green-600 to-green-800 text-white py-8 px-4">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-3xl font-bold mb-2">Italian Property Buyer Profile</h1>
          <p className="text-green-100">Help us understand your needs to provide personalized service</p>
        </div>
      </div>
